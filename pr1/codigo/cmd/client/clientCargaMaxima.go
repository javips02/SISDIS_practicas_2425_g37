...
func main() {
	args := os.Args
	if len(args) != 3 {
		log.Println("Error: endpoint missing: go run client.go ip:port <requestsEvery23Ms>")
		os.Exit(1)
	}
	endpoint := args[1]
	requestTmp, err := strconv.Atoi(args[2])
	com.CheckError(err)
	...
	tts := 23 // time to sleep between consecutive requests
